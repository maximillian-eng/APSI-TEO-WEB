...
<head>
    ...
    <style>
        ...
        .sidebar-member { background: linear-gradient(180deg, #064e3b 0%, #065f46 50%, #064e3b 100%); }
    </style>
</head>
<body class="font-sans antialiased bg-slate-50">
    <div x-data="{ open: false }" class="min-h-screen flex">
        ...
        <aside :class="open ? 'translate-x-0' : '-translate-x-full'"
               class="fixed inset-y-0 left-0 z-50 w-72 sidebar-member transform transition-transform duration-300 ease-out lg:translate-x-0 lg:static lg:z-auto flex flex-col">
            {{-- Brand --}}
            <div class="flex items-center gap-3 h-16 px-6 border-b border-emerald-500/20">
                ...
                <div>
                    ...
                    <span class="text-emerald-300/60 text-[10px] uppercase tracking-widest font-semibold">Area Anggota</span>
                </div>
                <button @click="open = false" class="lg:hidden ml-auto text-emerald-300 hover:text-white">
            ...
            <nav class="flex-1 overflow-y-auto py-4 px-3 space-y-1">
                <div class="px-3 mb-3">
                    <span class="text-[10px] uppercase tracking-widest text-emerald-300/50 font-semibold">Navigasi</span>
                </div>

                @php
                    $sidebarItems = [
                        ['route' => 'member.dashboard', 'name' => 'Dashboard', 'icon' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"/>'],
                        ['route' => 'member.katalog.index', 'name' => 'Katalog Buku', 'icon' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"/>'],
                        ['route' => 'member.riwayat', 'name' => 'Riwayat Peminjaman', 'icon' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4"/>'],
                    ];
                @endphp
                @foreach($sidebarItems as $item)
                    <a href="{{ route($item['route']) }}"
                       class="flex items-center gap-3 px-3 py-2.5 text-sm font-medium rounded-xl transition-all duration-200 {{ request()->routeIs($item['route'].'*') ? 'bg-white/15 text-white shadow-lg shadow-emerald-900/30' : 'text-emerald-200/70 hover:bg-white/5 hover:text-white' }}">
            ...
            <div class="p-4 border-t border-emerald-500/20">
                <div class="flex items-center gap-3">
                    <div class="w-9 h-9 bg-gradient-to-br from-emerald-400 to-emerald-600 rounded-xl flex items-center justify-center text-white text-sm font-bold shadow-lg shadow-emerald-500/20">
                        {{ substr(auth()->user()->nama, 0, 1) }}
                    </div>
                    <div class="flex-1 min-w-0">
                        <p class="text-sm font-semibold text-white truncate">{{ auth()->user()->nama }}</p>
                        <p class="text-xs text-emerald-300/50">Anggota</p>
                    </div>
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <button type="submit" class="p-2 text-emerald-300/50 hover:text-white hover:bg-white/10 rounded-lg transition" title="Logout">
