...
<div class="max-w-2xl mx-auto">
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
        <form method="POST" action="{{ route('admin.pengarang.update', $pengarang) }}">
            @csrf @method('PUT')
            <div class="space-y-4">
                <div>
                    <label for="nama_pengarang" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Nama Pengarang</label>
                    <input type="text" name="nama_pengarang" id="nama_pengarang" value="{{ old('nama_pengarang', $pengarang->nama_pengarang) }}" required
                ...
                <div>
                    ...
                    <textarea name="biografi" id="biografi" rows="4" required
                              class="mt-1 block w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 shadow-sm focus:border-purple-500 focus:ring-purple-500">{{ old('biografi', $pengarang->biografi) }}</textarea>
            ...
            <div class="flex items-center justify-end mt-6 space-x-3">
                ...
                <button type="submit" class="px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 text-sm font-medium">Simpan Perubahan</button>
            </div>
        </form>
    </div>
</div>
