        ...
        <form method="POST" action="{{ route('admin.transaksi.proses-pengembalian', $transaksi) }}">
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <div class="space-y-2">
                        <label class="flex items-center p-3 border rounded-lg cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700 {{ old('kondisi_buku') === 'Bagus' ? 'border-indigo-500 bg-indigo-50 dark:bg-indigo-900/20' : 'border-gray-300 dark:border-gray-600' }}">
                            <input type="radio" name="kondisi_buku" value="Bagus" {{ old('kondisi_buku', 'Bagus') === 'Bagus' ? 'checked' : '' }} class="text-indigo-600 focus:ring-indigo-500">
                            <span class="ml-3 text-sm text-gray-800 dark:text-gray-200">Bagus (Tidak ada denda kerusakan)</span>
                        </label>
                        <label class="flex items-center p-3 border rounded-lg cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700 {{ old('kondisi_buku') === 'Rusak Ringan' ? 'border-indigo-500 bg-indigo-50 dark:bg-indigo-900/20' : 'border-gray-300 dark:border-gray-600' }}">
                            <input type="radio" name="kondisi_buku" value="Rusak Ringan" {{ old('kondisi_buku') === 'Rusak Ringan' ? 'checked' : '' }} class="text-indigo-600 focus:ring-indigo-500">
                            <span class="ml-3 text-sm text-gray-800 dark:text-gray-200">Rusak Ringan (Denda: Rp 25.000)</span>
                        </label>
                        <label class="flex items-center p-3 border rounded-lg cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700 {{ old('kondisi_buku') === 'Rusak Berat' ? 'border-indigo-500 bg-indigo-50 dark:bg-indigo-900/20' : 'border-gray-300 dark:border-gray-600' }}">
                            <input type="radio" name="kondisi_buku" value="Rusak Berat" {{ old('kondisi_buku') === 'Rusak Berat' ? 'checked' : '' }} class="text-indigo-600 focus:ring-indigo-500">
            ...
            <div class="flex items-center justify-end mt-6 space-x-3">
                ...
                <button type="submit" class="px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 text-sm font-medium">Konfirmasi Pengembalian</button>
            </div>
        </form>
    </div>
</div>
