        ...
        <form method="POST" action="{{ route('admin.buku.update', $buku) }}">
            ...
            <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <select name="id_pengarang" id="id_pengarang" required
                            ...
                            @foreach($pengarangs as $pengarang)
                                <option value="{{ $pengarang->id }}" {{ old('id_pengarang', $buku->id_pengarang) == $pengarang->id ? 'selected' : '' }}>{{ $pengarang->nama_pengarang }}</option>
                            @endforeach
                        </select>
                ...
                <div>
                    ...
                    <input type="number" name="stok_total" id="stok_total" value="{{ old('stok_total', $buku->stok_total) }}" min="1" required
                           class="mt-1 block w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 shadow-sm focus:border-purple-500 focus:ring-purple-500">
                    @error('stok_total') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                    <p class="text-xs text-gray-500 mt-1">Stok tersedia saat ini: {{ $buku->stok_tersedia }}</p>
                </div>
            </div>
