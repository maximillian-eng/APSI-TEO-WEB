...
<div class="bg-white dark:bg-gray-800 rounded-lg shadow p-4 sm:p-6 mb-6">
    <form method="GET" class="flex flex-col sm:flex-row gap-3">
        ...
        <button type="submit" class="px-6 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 text-sm font-medium">Cari</button>
    </form>
</div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow overflow-hidden hover:shadow-lg transition-shadow">
        <div class="p-5">
            <div class="flex items-start justify-between mb-2">
                <span class="px-2 py-1 text-xs font-medium bg-indigo-100 text-indigo-800 dark:bg-indigo-900 dark:text-indigo-200 rounded-full">{{ $buku->kategori->nama_kategori }}</span>
            ...
            <div class="flex items-center justify-between mt-4">
                ...
                <a href="{{ route('member.katalog.show', $buku) }}" class="text-indigo-600 hover:text-indigo-800 dark:text-indigo-400 text-sm font-medium">
                    Detail &rarr;
                </a>
            </div>
        </div>
