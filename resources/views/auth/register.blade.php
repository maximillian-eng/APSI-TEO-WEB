<x-guest-layout>
    <form method="POST" action="{{ route('register') }}" class="space-y-5">
        ...
        <div>
            <label for="nama" class="block text-sm font-semibold text-gray-300 mb-1.5">Nama Lengkap</label>
            <div class="relative">
                ...
                <input id="nama" type="text" name="nama" :value="old('nama')" required autofocus autocomplete="name"
                       class="block w-full pl-11 pr-4 py-3 rounded-xl border-2 border-gray-600 bg-gray-700/50 text-white placeholder-gray-400 focus:border-amber-500 focus:ring-4 focus:ring-amber-500/10 transition-all duration-200"
                       placeholder="Nama lengkap Anda">
            </div>
            @error('nama') <p class="text-red-400 text-sm mt-1.5 flex items-center gap-1"><svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clip-rule="evenodd"/></svg>{{ $message }}</p> @enderror
        </div>
        ...
        <div>
            <label for="password_confirmation" class="block text-sm font-semibold text-gray-300 mb-1.5">Konfirmasi Password</label>
            <div class="relative">
                <div class="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none">
    ...
    <div class="mt-6 text-center">
        <p class="text-sm text-gray-400">
            Sudah punya akun?
            <a href="{{ route('login') }}" class="font-semibold text-amber-400 hover:text-amber-300 transition">Masuk</a>
        </p>
    </div>
</x-guest-layout>
