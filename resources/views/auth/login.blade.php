<x-guest-layout>
        ...
        <div class="flex items-center justify-between">
            <label for="remember_me" class="inline-flex items-center gap-2 cursor-pointer">
                <input id="remember_me" type="checkbox" name="remember"
                       class="w-4 h-4 rounded border-gray-500 text-amber-500 focus:ring-amber-500 transition">
                <span class="text-sm text-gray-400">Ingat saya</span>
            </label>

            @if (Route::has('password.request'))
                <a class="text-sm font-medium text-amber-400 hover:text-amber-300 transition" href="{{ route('password.request') }}">
                    Lupa password?
                </a>
            @endif
        </div>
    ...
    <div class="mt-6 text-center">
        <p class="text-sm text-gray-400">
            Belum punya akun?
            <a href="{{ route('register') }}" class="font-semibold text-amber-400 hover:text-amber-300 transition">Daftar sekarang</a>
        </p>
    </div>
</x-guest-layout>
