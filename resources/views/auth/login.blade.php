<x-guest-layout>
    ...
    <form method="POST" action="{{ route('login') }}" class="space-y-5">
        ...
        <div>
            ...
            <div class="relative">
                ...
                <input id="email" type="email" name="email" :value="old('email')" required autofocus autocomplete="username"
                       class="block w-full pl-11 pr-4 py-3 rounded-xl border-2 border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-700/50 text-gray-900 dark:text-gray-100 placeholder-gray-400 focus:border-amber-500 focus:ring-4 focus:ring-amber-500/10 transition-all duration-200"
        ...
        <div class="flex items-center justify-between">
            ...
            @if (Route::has('password.request'))
                <a class="text-sm font-medium text-amber-700 dark:text-amber-400 hover:text-amber-800 dark:hover:text-amber-300 transition" href="{{ route('password.request') }}">
                    Lupa password?
                </a>
            @endif
        </div>
    ...
    <div class="mt-6 text-center">
        <p class="text-sm text-gray-500 dark:text-gray-400">
            Belum punya akun?
            <a href="{{ route('register') }}" class="font-semibold text-amber-700 dark:text-amber-400 hover:text-amber-800 dark:hover:text-amber-300 transition">Daftar sekarang</a>
        </p>
    </div>
</x-guest-layout>
